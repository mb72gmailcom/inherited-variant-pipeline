...
class BlockWriter:
    ...
    HEADER = "#CHROM\tPOS\tID\tREF\tALT\tTRIO_CALLS\n"

    def __init__(self, path: Path, block_size: int = DEFAULT_BLOCK_SIZE) -> None:
        self.path = path
...
class ResultWriter:
    ...
    def __post_init__(self) -> None:
        ...
        self.inherited = BlockWriter(self.output_dir / "inherited.tsv", self.block_size)
        self.mendelian_bad = BlockWriter(self.output_dir / "mendelian_bad.tsv", self.block_size)
    ...
    def write_inherited(
        ...
        if not hits:
            return
        self.inherited.append(chrom, pos, ref, alt, serialize_trio_calls(hits))
    ...
    def write_mendelian_bad(
        ...
        if not hits:
            return
        self.mendelian_bad.append(chrom, pos, ref, alt, serialize_trio_calls(hits))
...
def read_result_tsv(path: Path) -> list[tuple[str, str, str, str, dict[str, tuple[str, str, str, str]]]]:
    """Parse a result TSV into (chrom, pos, ref, alt, hits) records."""
    records = []
